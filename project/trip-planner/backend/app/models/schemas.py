...
class TripRequest(BaseModel):
    city: str = Field(..., description="目的地城市", examples=["北京", "上海", "广州"])
    start_time: str = Field(..., description="旅行开始时间，格式为YYYY-MM-DD", examples=["2024-07-01"])
    end_time: str = Field(..., description="旅行结束时间，格式为YYYY-MM-DD", examples=["2024-07-07"])
    travel_days: int = Field(..., description="旅行天数", examples=[7])
    transportation: str = Field(..., description="交通方式", examples=["飞机", "火车", "汽车"])
    accommodation: str = Field(..., description="住宿类型", examples=["酒店", "民宿", "青年旅社"])
    preferences: Optional[List[str]] = Field(default=[], description="旅行偏好", examples=[["文化", "美食"], ["自然", "冒险"]])
    free_text_input: Optional[str] = Field(default="", description="自由文本输入，用户可以提供额外的旅行信息或特殊要求", examples=["我喜欢历史文化，希望有导游讲解。"])
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "accommodation": "酒店",
                "preferences": ["文化", "美食"],
                "free_text_input": "我喜欢历史文化，希望有导游讲解。"
            }
        }

class POISearchRequest(BaseModel):
    """POI搜索请求"""
    keywords: str = Field(..., description="搜索关键词", examples=["故宫", "天安门"])
    city: str = Field(..., description="城市", examples=["北京", "上海", "广州"])
